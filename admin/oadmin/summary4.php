...
$result=mysqli_query($db,'SELECT paperid, auid, opstatus, ptitle, trackid, sessionid, pfilename FROM papers WHERE trackid=0');
//
...
class PDF extends FPDF
   ...
   $sesid=mysqli_query($db,"SELECT * FROM sessions WHERE tid='$tid'");
	while($res=mysqli_fetch_array($sesid)){
		...
		$pdf->Cell(165,20,'Paper Title',1,0,'',true);
		$pdf->Cell(60,10,'Paper Status',1,0,'C',true);
		$pdf->Cell(30,20,'Overall Status',1,0,'',true);
		$pdf->Cell(0,10,'',0,1);
		$pdf->Cell(190,10,'',0,0);
...
//$sql="SELECT pid, auid, pstatus, ptitle, trackid, sessionid, pfilename FROM papers WHERE sessionid='$row->sid' ";
		$sql="SELECT paperid, auid, opstatus, ptitle, trackid, sessionid, pfilename, rewd1, rewd2, rewd3 FROM papers WHERE trackid='$tid' AND sessionid='$sid' ";
		$result1=mysqli_query($db,$sql);
		while($data=mysqli_fetch_array($result1)){
		$pdf->Ln(10);
		$pdf->Cell(25,10,$data['paperid'],1,0);
		//$pdf->Cell(7,10,$data['auid'],1,0);
		$pdf->Cell(165,10,$data['ptitle'],1,0);
		if($data['rewd1']==1)
		{
		...
		if($data['opstatus']=='Reviewed'){$pdf->Cell(30,10,'Decision Pending',1,0);}
					else{$pdf->Cell(30,10,$data['opstatus'],1,0);}
            ...
            <div class="row" style="padding:0 !important;">
                <div class="col-md-12" style="padding:0 !important;">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li><a href="summary4.php">Overall paper Review Summary</a></li>
...
<br/><br/>
	<div class="container" style="font-size:16px;">
		...
		$sql="SELECT paperid, auid, opstatus, ptitle, trackid, sessionid, pfilename,rewd1, rewd2, rewd3 FROM papers WHERE trackid='$tid' AND sessionid='$sid' AND opstatus!='NULL' ";
		$result=mysqli_query($db,$sql);
		while($row=mysqli_fetch_array($result))
		{
			echo '<tr>
					...
					<td>'.$row['paperid'].'</td>
					<td>'.$row['ptitle'].'</td>';
					if($row['rewd1']==1){echo '<td>Done</td>';}
					elseif($row['rewd1']==0) {echo '<td>Pending</td>';}
					else {echo '<td>--</td>';}
					if($row['rewd2']==1){echo '<td>Done</td>';}
					elseif($row['rewd2']==0) {echo '<td>Pending</td>';}
					else {echo '<td>--</td>';}
					if($row['rewd3']==1){echo '<td>Done</td>';}
					elseif($row['rewd3']==0) {echo '<td>Pending</td>';}
					else {echo '<td>--</td>';}
				if($row['opstatus']=='Reviewed'){echo '<td>Decision Pending</td></tr>';}
					else{echo '<td>'.$row['opstatus'].'</td></tr>';}
