...
if(isset($_POST["state_id"]) && !empty($_POST["state_id"])){
    //Get all city data
    $query = $db->query("SELECT * FROM papers WHERE sessionid = ".$_POST['state_id']." AND opstatus!='accepted' AND opstatus!='rejected' ");
...
<tr class=active>
  ...
  <?php      while($row = $query->fetch_assoc()){ ?>
			...
			<td > <strong><?php  if(($row['rewd1']==0)&&($row['rewd2']==0)&&($row['rewd3']==0)){$done = 0; echo "Pending";} else if($row['opstatus'] == 'reviewed'){$done = 1; echo "Done";} else{$done = 0; echo "Pending";}?></strong></td>
	
        					<td > <strong><?php  if($done == 1){
