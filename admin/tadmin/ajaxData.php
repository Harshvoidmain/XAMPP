...
<tr class=active>
  ...
  <?php      while($row = $query->fetch_assoc()){ ?>
			...
			<td > <strong><?php  if($row['rewd1']==0){echo "Pending";} else if($row['rewd1']==1){echo "Done";}?></strong></td>
	
        					<!--<td > <strong><?php  //if(($row['rewd1']==1)&&($row['rewd2']==1)&&($row['rewd3']==1)){echo "<button class='btn btn-info btn-md' data-title='View' name=view style='font-size:19px'>Send Decision</span></button>";}?></strong></td>
		--><td > <strong><?php if($row['rewd2']==0){echo "Pending";}else if ($row['rewd2']==1){echo "Done";}?></strong></td>
		<td > <strong><?php if($row['rewd3']==0){echo "Pending";} else if ($row['rewd3']==1){echo "Done";}?></strong></td>
