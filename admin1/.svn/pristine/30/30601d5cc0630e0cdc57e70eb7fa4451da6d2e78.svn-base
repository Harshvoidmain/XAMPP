...
class PaginatorComponent extends Component {
    ...
    public function paginate($object = null, $scope = array(), $whitelist = array()) {
         ...
         * filter adjustments
         */
        //checking if condition is set in url
        if (is_array($object)) {
            $whitelist = $scope;
            $scope = $object;
            $object = null;
        ...
        $options = $this->validateSort($object, $options[0], $whitelist);
        ...
        if ($object->hasMethod('paginate')) {
            $results = $object->paginate(
                    $conditions, $fields, $order, $limit, $page, $recursive, $extra, $contain
            );
        } else {
            if (!isset($joins)) {
                //forcing join for hasMany & $hasAndBelongsToMany
                $joins = array();
                $associatedModels = $object->getAssociated();

                $baseModel = $object->alias;
                foreach ($associatedModels as $model => $relation) {
                    //escapeing for belongsTo and hasOne
                    if (in_array($relation, array('belongsTo', 'hasOne'))) {
                        continue;
                    }
                    //model
                    $modelAttribute = $object->{$relation}[$model];
                    $foreignKey = strtolower(Inflector::underscore($model)) . '_id';
                    if (isset($modelAttribute['foreignKey'])) {
                        $foreignKey = $modelAttribute['foreignKey'];
                    }
                    $associationForeignKey = 'id';
                    if (isset($modelAttribute['associationForeignKey'])) {
                        $associationForeignKey = $modelAttribute['associationForeignKey'];
                    }
                    //hasMany
                    if ($relation === 'hasMany') {
                        array_push($joins, array(
                            'table' => isset($modelAttribute['joinTable']) ? $modelAttribute['joinTable'] : strtolower(Inflector::underscore(Inflector::pluralize($model))),
                            'type' => 'LEFT',
                            'alias' => isset($modelAttribute['className']) ? $modelAttribute['className'] : $model,
                            'conditions' => array(
                                "$model.$foreignKey" => "$baseModel.$associationForeignKey"
                            )
                        ));
                    }
                    //hasAndBelongsToMany
                    if ($relation === 'hasAndBelongsToMany') {
                        $intermediateModel = Inflector::camelize(Inflector::singularize($modelAttribute['joinTable']));
                        array_push($joins, array(
                            'table' => $modelAttribute['joinTable'],
                            'type' => 'LEFT',
                            'alias' => $intermediateModel,
                            'conditions' => array(
                                "$baseModel.id" => "$intermediateModel.$foreignKey"
                            )
                        ));
                        $relatedModel = Inflector::camelize(Inflector::singularize($modelAttribute['className']));
                        array_push($joins, array(
                            'table' => Inflector::pluralize(Inflector::underscore($relatedModel)),
                            'type' => 'LEFT',
                            'alias' => $relatedModel,
                            'conditions' => array(
                                "$relatedModel.id" => "$intermediateModel.$associationForeignKey"
                            )
                        ));
                    }
                }
            ...
            $parameters = compact('conditions', 'fields', 'order', 'limit', 'page', 'joins');
    ...
    public function mergeOptions($alias) {
        ...
        $contains = array();
        $conditions = array();
        if (isset($this->Controller->request->params['named']['conditions']) && !empty($this->Controller->request->params['named']['conditions'])) {

            $split = explode('&', $this->Controller->request->params['named']['conditions']);

            foreach ($split as $key => $value) {
                $splitPara = explode('=', $value);
                $splitPara[0] = trim($splitPara[0]);
                $splitPara[1] = trim($splitPara[1]);
                //checking if condition or conatin
                if (substr($splitPara[0], 0, strlen($this->Controller->modelClass)) === $this->Controller->modelClass) {
                    $conditions[$splitPara[0] . ' LIKE'] = '%' . $splitPara[1] . '%';
                } else {
                    array_push($contains, ($splitPara[0] . ' LIKE "%' . $splitPara[1] . '%"'));
                }
            }
            $conditions = array('AND' => $conditions);
            unset($this->Controller->request->params['named']['conditions']);
        }
        ...
        $contains_conditions = array('conditions' => $conditions, 'contain' => array('OR' => $contains));
        array_push($contains_conditions, array_merge($defaults, $request));
        return $contains_conditions;
    }
