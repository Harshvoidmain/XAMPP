...
class FormHelper extends AppHelper {
     ...
     * class varibales
     */
    public $outerDivClass = 'control-group';
    public $labelClass = 'control-label';
    public $innerDivClass = 'controls';
    public $inputClass = 'm-wrap large';
    ...
    public function error($field, $text = null, $options = array()) {
        ...
        if ($options['wrap']) {
            ...
            unset($options['wrap']);
            if (!empty($error)) {
                $error = '<button class="close" data-dismiss="alert"></button>' . $error;
            }
    ...
    public function input($fieldName, $options = array()) {
        //mubasshir
        if (in_array($fieldName, array('file', 'profile_picture', 'product_photo_1', 'product_photo_2', 'product_photo_3', 'product_photo_4', 'product_photo_5'))) {
            $options['type'] = 'file';
        }
        ...
        if (class_exists($modelKey)) {
            App::import("Model", $modelKey);
            $Model = new $modelKey();

            /**
             * checking if its a required field
             */
            if (isset($Model->validate[$fieldName]) && !empty($Model->validate[$fieldName])) {
                $fieldValidate = array_values($Model->validate[$fieldName]);
                $required = isset($fieldValidate[0]['required']) && $fieldValidate[0]['required'];
            }
            /**
            ...
            $class = '';
            if (strpos($options['class'], 'no_validate') === FALSE) {
                if (isset($Model->validationEngine[$fieldName])) {
                    if (is_string($Model->validationEngine[$fieldName])) {
                        $Model->validationEngine[$fieldName] = array($Model->validationEngine[$fieldName]);
                    }
                }
                $validate = ' validate[' . (isset($Model->validationEngine[$fieldName]) ? implode(',', $Model->validationEngine[$fieldName]) : '') . '' . $class . ']';
        ...
        if (isset($options['class'])) {
            ...
            if (strpos($options['class'], 'no-large') !== FALSE) {
                $options['class'] = str_replace(' large', ' ', $options['class']);
            }
        } else {
            $options['class'] = $this->inputClass;
        }
        $out['input'] = '<div class="' . $this->innerDivClass . '">' . $this->_getInput(compact('type', 'fieldName', 'options', 'radioOptions', 'selected', 'dateFormat', 'timeFormat'));
    ...
    protected function _magicOptions($options) {
        ...
        } elseif (in_array($fieldKey, array('website'))) {
            $options['type'] = 'url';
        } elseif (in_array($fieldKey, array('tel', 'telephone', 'phone', 'contact_number', 'contact_number_1', 'contact_number_2'))) {
            $options['type'] = 'tel';
        } elseif (in_array($fieldKey, array('email', 'email_address', 'email_address_1', 'email_address_2'))) {
            $options['type'] = 'email';
    ...
    protected function _divOptions($options) {
        ...
        $divOptions = array('class' => $this->outerDivClass);
    ...
    protected function _inputLabel($fieldName, $label, $options) {
        ...
        //checking if we need to add required
        if ($options['required']) {
            $label = str_replace('</label>', '<span class="required">*</span></label>', $label);
        }
        return $label;
    }
    ...
    public function submit($caption = null, $options = array()) {
        ...
        $out = '<button type="submit" class="btn blue"><i class="icon-ok"></i> ' . $options['value'] . '</button>
                <button type="reset" class="btn"><i class="icon-remove"></i> Clear</button>';
        if (isset($divOptions)) {
            $tag = $divOptions['tag'];
            unset($divOptions['tag']);
