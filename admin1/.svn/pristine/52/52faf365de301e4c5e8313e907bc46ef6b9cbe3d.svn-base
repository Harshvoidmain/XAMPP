...
class Inflector {
    ...
    /**
     * returns in singulized humanized camelcase word
     */
    public static function shc($word = '') {
        return Inflector::singularize(Inflector::humanize(Inflector::underscore($word)));
    }

    /**
     * returns in pluralized humanized camelcase word
     */
    public static function phc($word = '') {
        return Inflector::pluralize(Inflector::humanize(Inflector::underscore($word)));
    }
    
    public static function modelized($word=''){
         return Inflector::pluralize(Inflector::camelize(Inflector::underscore(str_replace('-', '_', $word))));
    }
     ...
     * @param type $id
     * @param type $name
     * @return type string returns /u/1/mubasshir-pawle
     */
    public static function urlified($id = 0, $name = 'Anonymous', $model = 'Faculty', $prefix = '', $postfix = '') {
        $modelToUsRL = array('Faculty'=>'faculty');
        return '/' . $prefix . $modelToUsRL[$model] . $postfix . '/' . $id . '/' . preg_replace('![^a-z0-9]+!i', '-', strtolower(Inflector::underscore($name)));
    }
