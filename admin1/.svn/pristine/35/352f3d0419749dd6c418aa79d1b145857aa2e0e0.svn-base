...
<div class="row-fluid">
    <div class="span12 responsive" data-tablet="span12 fix-offset" data-desktop="span12">
        ...
        <div class="portlet box grey">
            <div class="portlet-title">
                <div class="caption"><i class="icon-user"></i><?php echo __('Committees'); ?></div>
                <div class="actions">
                    <?php if($this->Permissions->hasAccess($this->name, 'add')){?>
	                    <?php echo $this->Html->link('<i class="icon-plus"></i> Add '.__('Committee'),array('action'=>'add'), array('class'=>'btn blue', 'escape'=>FALSE));?>
                    <?php } ?>
                    <div class="btn-group">
                            ...
                            <thead>
                                <tr class="row">
                                    <th style="width:8px;"><input type="checkbox" class="group-checkable" data-set="#sample_2 .checkboxes" /></th>
                                                                            ...
                                                                            <th class="hidden-480 sorting"><?php echo $this->Paginator->sort('member'); ?></th>
                                                                            <th class="hidden-480 sorting"><?php echo $this->Paginator->sort('designation'); ?></th>
                                                                            <th class="hidden-480 sorting"><?php echo $this->Paginator->sort('post'); ?></th>
                                                                            <th class="hidden-480 sorting"><?php echo $this->Paginator->sort('image'); ?></th>
	...
	<tr class="odd gradeX row">
		<td><input type="checkbox" class="checkboxes" value="<?php echo h($committee['Committee']['id']); ?>" /></td>
		<td class="hidden-480"><?php echo h($committee['Committee']['id']); ?>&nbsp;</td>
		<td class="hidden-480"><?php echo $this->Common->get_button($committee['Committee']['name'], array('Agnel Marathi Cultural Committee','Anti-Ragging Committee','Women Grievance Redressal Committee','Grievance Redressal Committee','Women Cell')); ?>&nbsp;</td>
		<td class="hidden-480"><?php echo h($committee['Committee']['member']); ?>&nbsp;</td>
		<td class="hidden-480"><?php echo h($committee['Committee']['designation']); ?>&nbsp;</td>
		<td class="hidden-480"><?php echo h($committee['Committee']['post']); ?>&nbsp;</td>
		<td class="hidden-480"><?php echo h($committee['Committee']['image']); ?>&nbsp;</td>
		<td class="hidden-480"><?php echo $this->Common->get_button($committee['Committee']['created_by'], NULL); ?>&nbsp;</td>
		<td class="hidden-480"><?php echo h($committee['Committee']['created_timestamp']); ?>&nbsp;</td>
		<td class="actions hidden-480">
<?php if($this->Permissions->hasAccess($this->name, 'view')){?>
				<?php echo $this->Html->link(__('<i class="icon-eye-open"></i> View'), array('action' => 'view', $committee['Committee']['id']), array('class'=>'btn purple','escape'=>false)); ?>
<?php } ?>
<?php if($this->Permissions->hasAccess($this->name, 'edit')){?>
				<?php echo $this->Html->link(__('<i class="icon-pencil"></i> Edit'), array('action' => 'edit', $committee['Committee']['id']), array('class'=>'btn green','escape'=>false)); ?>
<?php } ?>
<?php if($this->Permissions->hasAccess($this->name, 'delete')){?>
				<?php echo $this->Form->postLink(__('<i class="icon-trash"></i> Delete'), array('action' => 'delete', $committee['Committee']['id']), array('class'=>'btn red','escape'=>false), __('Are you sure you want to delete # %s?', $committee['Committee']['id'])); ?>
<?php } ?>
		</td>
	</tr>
<?php endforeach; ?>
