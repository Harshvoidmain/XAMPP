    ...
    <div class="span12 responsive" data-tablet="span12 fix-offset" data-desktop="span12">
        ...
        <div class="portlet box grey">
            <div class="portlet-title">
                <div class="caption"><i class="icon-user"></i><?php echo __('Keynotes'); ?></div>
                <div class="actions">
                    <?php if ($this->Permissions->hasAccess($this->name, 'add')) { ?>
                        <?php echo $this->Html->link('<i class="icon-plus"></i> Add ' . __('Keynote'), array('action' => 'add'), array('class' => 'btn blue', 'escape' => FALSE)); ?>
                    <?php } ?>
                    <div class="btn-group">
                            ...
                            <tbody>
                                <?php foreach ($keynotes as $keynote): ?>
                                    <tr class="odd gradeX row">
                                        <td><input type="checkbox" class="checkboxes" value="<?php echo h($keynote['Keynote']['id']); ?>" /></td>
                                        <td class="hidden-480"><?php echo h($keynote['Keynote']['id']); ?>&nbsp;</td>
                                        <td class="hidden-480"><?php echo h($keynote['Keynote']['sr_no']); ?>&nbsp;</td>
                                        <td class="hidden-480"><?php echo h($keynote['Keynote']['name']); ?>&nbsp;</td>
                                        <td class="hidden-480"><?php echo h($keynote['Keynote']['designation']); ?>&nbsp;</td>
                                        <td class="hidden-480"><?php echo h($keynote['Keynote']['description']); ?>&nbsp;</td>
                                        <td class="actions hidden-480">
                                            <?php if ($this->Permissions->hasAccess($this->name, 'view')) { ?>
                                                <?php echo $this->Html->link(__('<i class="icon-eye-open"></i> View'), array('action' => 'view', $keynote['Keynote']['id']), array('class' => 'btn purple', 'escape' => false)); ?>
                                            <?php } ?>
                                            <?php if ($this->Permissions->hasAccess($this->name, 'edit')) { ?>
                                                <?php echo $this->Html->link(__('<i class="icon-pencil"></i> Edit'), array('action' => 'edit', $keynote['Keynote']['id']), array('class' => 'btn green', 'escape' => false)); ?>
                                            <?php } ?>
                                            <?php if ($this->Permissions->hasAccess($this->name, 'delete')) { ?>
                                                <?php echo $this->Form->postLink(__('<i class="icon-trash"></i> Delete'), array('action' => 'delete', $keynote['Keynote']['id']), array('class' => 'btn red', 'escape' => false), __('Are you sure you want to delete # %s?', $keynote['Keynote']['id'])); ?>
                                            <?php } ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
