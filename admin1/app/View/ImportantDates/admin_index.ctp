    ...
    <div class="span12 responsive" data-tablet="span12 fix-offset" data-desktop="span12">
        ...
        <div class="portlet box grey">
            <div class="portlet-title">
                <div class="caption"><i class="icon-user"></i><?php echo __('ImportantDates'); ?></div>
                <div class="actions">
                    <?php if ($this->Permissions->hasAccess($this->name, 'add')) { ?>
                        <?php echo $this->Html->link('<i class="icon-plus"></i> Add ' . __('ImportantDate'), array('action' => 'add'), array('class' => 'btn blue', 'escape' => FALSE)); ?>
                    <?php } ?>
                    <div class="btn-group">
                        ...
                        <table id="sample_2" class="table table-striped table-bordered table-hover dataTable" aria-describedby="sample_2_info">
                            <thead>
                                <tr class="row">
                                    ...
                                    <th class="hidden-480 sorting"><?php echo $this->Paginator->sort('date'); ?></th>
                                    
                                    
                                    <th class="hidden-480 sorting"><?php echo $this->Paginator->sort('event'); ?></th>
                            ...
                            <tbody>
                                <?php foreach ($importantDates as $importantDate): ?>
                                    <tr class="odd gradeX row">
                                        <td><input type="checkbox" class="checkboxes" value="<?php echo h($importantDate['ImportantDate']['id']); ?>" /></td>
                                        <td class="hidden-480"><?php echo h($importantDate['ImportantDate']['id']); ?>&nbsp;</td>
                                        <td class="hidden-480"><?php echo h($importantDate['ImportantDate']['date']); ?>&nbsp;</td>
                                        
                                        <td class="hidden-480"><?php echo h($importantDate['ImportantDate']['event']); ?>&nbsp;</td>
                                        <td class="actions hidden-480">
                                            <?php if ($this->Permissions->hasAccess($this->name, 'view')) { ?>
                                                <?php echo $this->Html->link(__('<i class="icon-eye-open"></i> View'), array('action' => 'view', $importantDate['ImportantDate']['id']), array('class' => 'btn purple', 'escape' => false)); ?>
                                            <?php } ?>
                                            <?php if ($this->Permissions->hasAccess($this->name, 'edit')) { ?>
                                                <?php echo $this->Html->link(__('<i class="icon-pencil"></i> Edit'), array('action' => 'edit', $importantDate['ImportantDate']['id']), array('class' => 'btn green', 'escape' => false)); ?>
                                            <?php } ?>
                                            <?php if ($this->Permissions->hasAccess($this->name, 'delete')) { ?>
                                                <?php echo $this->Form->postLink(__('<i class="icon-trash"></i> Delete'), array('action' => 'delete', $importantDate['ImportantDate']['id']), array('class' => 'btn red', 'escape' => false), __('Are you sure you want to delete # %s?', $importantDate['ImportantDate']['id'])); ?>
                                            <?php } ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
