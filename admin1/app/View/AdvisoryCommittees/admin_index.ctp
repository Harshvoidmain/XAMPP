    ...
    <div class="span12 responsive" data-tablet="span12 fix-offset" data-desktop="span12">
        ...
        <div class="portlet box grey">
            <div class="portlet-title">
                <div class="caption"><i class="icon-user"></i><?php echo __('AdvisoryCommittees'); ?></div>
                <div class="actions">
                    <?php if ($this->Permissions->hasAccess($this->name, 'add')) { ?>
                        <?php echo $this->Html->link('<i class="icon-plus"></i> Add ' . __('AdvisoryCommittee'), array('action' => 'add'), array('class' => 'btn blue', 'escape' => FALSE)); ?>
                    <?php } ?>
                    <div class="btn-group">
                ...
                <div role="grid" class="dataTables_wrapper form-inline" id="sample_2_wrapper">
                    ...
                    <div class="dataTables_scrollBody" style="overflow: auto; width: 100%;">
                        <table id="sample_2" class="table table-striped table-bordered table-hover dataTable" aria-describedby="sample_2_info">
                            ...
                            <tbody>
                                <?php foreach ($advisoryCommittees as $advisoryCommittee): ?>
                                    <tr class="odd gradeX row">
                                        <td><input type="checkbox" class="checkboxes" value="<?php echo h($advisoryCommittee['AdvisoryCommittee']['id']); ?>" /></td>
                                        <td class="hidden-480"><?php echo h($advisoryCommittee['AdvisoryCommittee']['id']); ?>&nbsp;</td>
                                        <td class="hidden-480"><?php echo h($advisoryCommittee['AdvisoryCommittee']['sr_no']); ?>&nbsp;</td>
                                        <td class="hidden-480"><?php echo h($advisoryCommittee['AdvisoryCommittee']['name']); ?>&nbsp;</td>
                                        
                                        <td class="hidden-480"><?php echo h($advisoryCommittee['AdvisoryCommittee']['designation']); ?>&nbsp;</td>
                                        <td class="hidden-480"><?php echo h($advisoryCommittee['AdvisoryCommittee']['institute']); ?>&nbsp;</td>
                                        
                                        
                                        <td class="actions hidden-480">
                                            <?php if ($this->Permissions->hasAccess($this->name, 'view')) { ?>
                                                <?php echo $this->Html->link(__('<i class="icon-eye-open"></i> View'), array('action' => 'view', $advisoryCommittee['AdvisoryCommittee']['id']), array('class' => 'btn purple', 'escape' => false)); ?>
                                            <?php } ?>
                                            <?php if ($this->Permissions->hasAccess($this->name, 'edit')) { ?>
                                                <?php echo $this->Html->link(__('<i class="icon-pencil"></i> Edit'), array('action' => 'edit', $advisoryCommittee['AdvisoryCommittee']['id']), array('class' => 'btn green', 'escape' => false)); ?>
                                            <?php } ?>
                                            <?php if ($this->Permissions->hasAccess($this->name, 'delete')) { ?>
                                                <?php echo $this->Form->postLink(__('<i class="icon-trash"></i> Delete'), array('action' => 'delete', $advisoryCommittee['AdvisoryCommittee']['id']), array('class' => 'btn red', 'escape' => false), __('Are you sure you want to delete # %s?', $advisoryCommittee['AdvisoryCommittee']['id'])); ?>
                                            <?php } ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
