    ...
    <div class="span12 responsive" data-tablet="span12 fix-offset" data-desktop="span12">
        ...
        <div class="portlet box grey">
            <div class="portlet-title">
                <div class="caption"><i class="icon-user"></i><?php echo __('Organizing Committees'); ?></div>
                <div class="actions">
                    <?php if ($this->Permissions->hasAccess($this->name, 'add')) { ?>
                        <?php echo $this->Html->link('<i class="icon-plus"></i> Add ' . __('Organizing Committee'), array('action' => 'add'), array('class' => 'btn blue', 'escape' => FALSE)); ?>
                    <?php } ?>
                    <div class="btn-group">
                ...
                <div role="grid" class="dataTables_wrapper form-inline" id="sample_2_wrapper">
                    ...
                    <div class="dataTables_scrollBody" style="overflow: auto; width: 100%;">
                        <table id="sample_2" class="table table-striped table-bordered table-hover dataTable" aria-describedby="sample_2_info">
                            ...
                            <tbody>
                                <?php foreach ($organizingCommittees as $organizingCommittee): ?>
                                    <tr class="odd gradeX row">
                                        <td><input type="checkbox" class="checkboxes" value="<?php echo h($organizingCommittee['OrganizingCommittee']['id']); ?>" /></td>
                                        <td class="hidden-480"><?php echo h($organizingCommittee['OrganizingCommittee']['id']); ?>&nbsp;</td>
                                        <td class="hidden-480"><?php echo h($organizingCommittee['OrganizingCommittee']['sr_no']); ?>&nbsp;</td>
                                        <td class="hidden-480"><?php echo h($organizingCommittee['OrganizingCommittee']['name']); ?>&nbsp;</td>
                                        
                                        <td class="actions hidden-480">
                                            <?php if ($this->Permissions->hasAccess($this->name, 'view')) { ?>
                                                <?php echo $this->Html->link(__('<i class="icon-eye-open"></i> View'), array('action' => 'view', $organizingCommittee['OrganizingCommittee']['id']), array('class' => 'btn purple', 'escape' => false)); ?>
                                            <?php } ?>
                                            <?php if ($this->Permissions->hasAccess($this->name, 'edit')) { ?>
                                                <?php echo $this->Html->link(__('<i class="icon-pencil"></i> Edit'), array('action' => 'edit', $organizingCommittee['OrganizingCommittee']['id']), array('class' => 'btn green', 'escape' => false)); ?>
                                            <?php } ?>
                                            <?php if ($this->Permissions->hasAccess($this->name, 'delete')) { ?>
                                                <?php echo $this->Form->postLink(__('<i class="icon-trash"></i> Delete'), array('action' => 'delete', $organizingCommittee['OrganizingCommittee']['id']), array('class' => 'btn red', 'escape' => false), __('Are you sure you want to delete # %s?', $organizingCommittee['OrganizingCommittee']['id'])); ?>
                                            <?php } ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
