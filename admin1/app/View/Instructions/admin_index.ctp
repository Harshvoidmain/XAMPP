                            ...
                            <tbody>
                                <?php foreach ($instructions as $instruction): ?>
                                    <tr class="odd gradeX row">
                                        <td><input type="checkbox" class="checkboxes" value="<?php echo h($instruction['Instruction']['id']); ?>" /></td>
                                        <td class="hidden-480"><?php echo h($instruction['Instruction']['id']); ?>&nbsp;</td>
                                        <td class="hidden-480"><?php echo h($instruction['Instruction']['instruction']); ?>&nbsp;</td>
                                        
                                        
                                        
                                        <td class="actions hidden-480">
                                            <?php if ($this->Permissions->hasAccess($this->name, 'view')) { ?>
                                                <?php echo $this->Html->link(__('<i class="icon-eye-open"></i> View'), array('action' => 'view', $instruction['Instruction']['id']), array('class' => 'btn purple', 'escape' => false)); ?>
                                            <?php } ?>
                                            <?php if ($this->Permissions->hasAccess($this->name, 'edit')) { ?>
                                                <?php echo $this->Html->link(__('<i class="icon-pencil"></i> Edit'), array('action' => 'edit', $instruction['Instruction']['id']), array('class' => 'btn green', 'escape' => false)); ?>
                                            <?php } ?>
                                            <?php if ($this->Permissions->hasAccess($this->name, 'delete')) { ?>
                                                <?php echo $this->Form->postLink(__('<i class="icon-trash"></i> Delete'), array('action' => 'delete', $instruction['Instruction']['id']), array('class' => 'btn red', 'escape' => false), __('Are you sure you want to delete # %s?', $instruction['Instruction']['id'])); ?>
                                            <?php } ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
