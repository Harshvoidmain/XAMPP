...
if(isset($_POST['printex']))
{
     header("Content-type:application/vnd.ms-word");
    header("Content-Disposition:attachment; Filename=excellence_awards.doc");
    header("Pragma:no-cache");
    //echo '<script>document.getElementByName("plenary");</script>';
    echo '<h1>Exacellence Awards<h1>';
    $id=$_POST['id'];
     $query="SELECT * FROM excellence_awards WHERE ID='$id' ";
		...
		<th>Phone</th></tr>';
        while($data1=mysqli_fetch_array($result1)){
        echo '<tr><td type="hidden"  value="'.$data1['ID'].'"><td>'.$data1['ID'].'</td>
            <td>'.$data1['fname'].'</td>
            <td>'.$data1['lname'].'</td>
			<td>'.$data1['oname'].'</td>
			<td>'.$data1['email'].'</td>
        ...
        $filename=$data1['attach_declaration'];
        $filepath="../../uploads/excellence_awards_submission/".$filename."";
