...
class ContainableBehavior extends ModelBehavior {
    ...
    public function containments(Model $Model, $contain, $containments = array(), $throwErrors = null) {
        ...
        //mubasshir
        /**
         * checking if OR condition is set
         *
         */
        $OR_condition_set = FALSE;
        if (isset($contain['OR'])) {
            $OR_condition_set = TRUE;
            $contain = $contain['OR'];
        }
        foreach ((array) $contain as $name => $children) {
            ...
            //for or condition
            if ($OR_condition_set) {
                foreach ($keep as $model => $conditions) {
                    if (isset($conditions['conditions']) && !empty($conditions['conditions'])) {
                        $keep[$model]['conditions'] = array('OR' => $conditions['conditions']);
                    }
                }
            }
